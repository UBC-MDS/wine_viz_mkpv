...
Other feedback included small detail changes that can help the user better navigate our app (like small label or hover-tooltip changes) that improved the overall UI of our app that aided in faster understanding of the app for the user.
Given the timeframe that we had to improve the app, most but not all changes were able to be made. Almost all of the functionality of the app that included filters was implemented, along with small design tweaks that made for a better looking app and certain plot changes. 
What could not be implemented given the time constraint was to make our work map and our variety bar chart interact-able (clickable) and to act as a filter. This can perhaps be moved to milestone 4.
The feedback given about the specific filters and the over-plotting of our scatterplot was the most valuable in that it shows that the user can be confused or overwhelmed by the data if they are looking at it for the first time, unlike us who have familiarity with what we built. This was very help in us constructing a price category that reduces the over-plotting and switching the scatter to a swarm plot. 
...
A lot of positive changes have been made to our app since milestone 2. The overall app is completely function (aside from the selectable map) and is easily navigable with instructions and intuitive error messages. It has an overall better sleek design with near concise labels and little to no redundancy. 
